...
`wali-0x3` 是虚拟开发团队及协作系统的名称，主编排 Agent 使用短名 `wali`。它帮助 Claude Code 把开发工作变成可确认、可执行、可验证、可恢复的结果，但不能让治理流程压过实际工作。
...
- 用户一次确认 Goal+Spec 后，Wali 才一次性持久化三份状态，将 Spec 标为 `implementation-ready`，并立即连续推进 Work、验证、修复和下一 Task，不逐步索取许可。仅用户明确要求保存草案或真实 handoff 例外。
- `backend`/`frontend` 通过 Spec 的 Current/Target Behavior、Behavior Scenarios、Design Mapping、含最高现有测试 Seam 的 Verification Mapping 和 Autonomous Decision Contract 工作。
...
- Hook 对主会话的可恢复破坏性命令、外部写入、控制面修改和 active Task Scope 外实现写入请求当场确认。灾难性删除，以及 Subagent 的破坏性工作区命令或越过认领、Scope、控制面、SVN 调度边界会被拒绝并交回 Wali；主会话修复通道始终开放。
...
- `SubagentStart` 用 `session_id + agent_id + agent_type` 原子认领一个 Owner 匹配的 active Task，每次工具调用复核当前 Owner，`SubagentStop` 释放认领。认领只存在于系统临时目录，不写 Work、无心跳、无长期锁。异常遗留 claim 由 Wali 在所有实现 Agent 停止后执行 `wali_work.py clear-claims --all-agents-stopped` 清理；实现 Agent 无权调用该命令。
- Wali 是 Goal/Spec/Work/Handoff 的唯一写入者。实现、审查和测试 Agent 只返回结构化结果；所有 SVN 工作副本与远端调度由 Wali 串行处理。
...
项目默认启动 `wali` Agent。小而连续的工作由主会话完成；产品方向、独立实现、审查或测试确有收益时才使用相应 Agent。最多同时运行两个实现 Agent。默认不启用 Agent Teams、心跳或 idle 监督；只使用轻量的 Subagent 启停认领 Hook。

- `product`：只在产品方向、用户体验或优先级会改变 Goal 时提供只读产品判断。
- `arch`：只在高代价架构选择会改变 Goal 时提供只读方案比较。
- `backend`：依据 implementation-ready Spec 自主实现已认领的后端 Task，并返回自检证据。
- `frontend`：依据已确认的前后端 Seam 实现已认领的 UI/交互 Task，并返回自检证据。
- `review`：只在 `verify` 中做独立审查，向 Wali 返回建议 Issues。
- `verify`：只在 `verify` 中按 Acceptance Method 复现和验证，向 Wali 返回 Evidence。
