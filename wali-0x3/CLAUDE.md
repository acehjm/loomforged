...
- `python3 .claude/hooks/wali_board.py --project-root . --open`：启动每 1 秒轮询一次的本地只读项目看板。
- `python3 .claude/hooks/wali_stop.py --project-root .`：检查是否可以停止。
- `python3 -m unittest -v test_wali_graph.py test_wali_policy.py test_wali_stop.py test_wali_supervision.py test_wali_svn.py test_wali_doctor.py test_wali_board.py`：在 `.claude/hooks/` 中运行 WALI 回归测试。
