...
- 开始时读取 `CLAUDE.md`、五个 WALI 状态文件、用户指定的资料、相关代码和项目配置；只做读取型 SVN 检查。
...
4. 识别当前 Goal 适用的项目 Rules 与 Refs，只读取相关条目并核对来源、版本、适用范围和真实代码。把稳定标识与本 Goal 的选择写入 `spec.md`；硬性依赖、禁用项和强制模板由 Rule 表达，详细兼容矩阵、模板正文和示例保留在 Ref。
