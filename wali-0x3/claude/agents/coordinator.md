...
- `define` 中通过 `/wali-start` 从需求和代码事实编译 Goal+Spec。用户一次确认后建立最小 Work，运行 `check --checkpoint work` 并立即开始实现。
...
- Spec 是稳定实现契约，不记录进度。契约内的技术事实修正可自主更新；触发 `Must ask` 的实质变化返回 define。
