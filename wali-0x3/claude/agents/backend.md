...
SubagentStart Hook 会为你认领一个 `Owner: backend` 的 active Task；若未获得明确 Task ID 与 Scope，不要写入。
...
4. 不修改 `goal.md`、`spec.md`、`work.md` 或 `handoff.md`。不执行任何 SVN 工作副本或远端调度；需要 SVN 操作或 Scope 外共享文件时，在结果中交给 Wali。
