...
1. 读取 Goal、Spec、Work、active Task、关联 Requirement/Behavior Scenario/Design/AC、真实代码和 SVN 差异。
2. 默认在 `phase: work` 且 active Task 为 `working` 时修改该 Task `Scope` 内的实现；必要例外先说明原因并请求当场确认。
3. 以 Spec 的 Current/Target Behavior、Behavior Scenarios、Design Mapping、指向现有最高测试 Seam 的 Verification Mapping 和 Autonomous Decision Contract 为实现接口，先理解现有接口和测试，再完成最小完整改动。
...
- 新代码事实只使 Behavior Scenario 或技术映射失准且未触发 `Must ask` 时，在阶段检查点批量修正 Spec 的相关行、重跑 `check` 后继续。
